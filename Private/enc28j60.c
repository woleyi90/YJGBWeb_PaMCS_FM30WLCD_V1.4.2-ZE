...
//	temp=*(vu32*)(0x1FFFF7E8);	//获取STM32的唯一ID的前24位作为MAC地址后三字节
	enc28j60_dev.macaddr[0]=g_DI.MAC[0];
	enc28j60_dev.macaddr[1]=g_DI.MAC[1];
	enc28j60_dev.macaddr[2]=g_DI.MAC[2];
	enc28j60_dev.macaddr[3]=g_DI.MAC[3];	//低三字节用STM32的唯一ID
	enc28j60_dev.macaddr[4]=g_DI.MAC[4];
	enc28j60_dev.macaddr[5]=g_DI.MAC[5];
	...
	//1 = 符合格式匹配条件的数据包将被接受
	//0 = 禁止过滤器
	ENC28J60_Write(ERXFCON,ERXFCON_UCEN|ERXFCON_CRCEN|ERXFCON_PMEN|ERXFCON_BCEN);//|ERXFCON_BCEN 添加UDP广播
	...
	ENC28J60_Write(ECOCON, 0x02); // change clkout from 6.25MHz to 12.5MHz
	return 0;
}
...
void ENC28J60_ISRHandler(void)
{
	...
	u8 packetnum;
	u16 temp;		  
	if(Sys_Flag.Bit.fETH_INT==TRUE)	//当网络接收到数据时，会产生中断
	{
		Sys_Flag.Bit.fETH_INT=FALSE;
		...
		if(status&EIR_RXERIF)		//接收错误中断标志位
		{
			...
			ENC28J60_Write_Op(ENC28J60_BIT_FIELD_SET,ECON2,ECON2_PKTDEC);
			printf("Receive error! Receives a packet number:%d\r\n",packetnum);
			printf("ENC28J60_Init ENC28J60_ISRHandler......\r\n");
			//ENC28J60_Init();		//初始化ENC28J60
			printf("Rev SoftReset......\r\n");
			SoftReset();
			//printf("ENC28J60_Init ok......\r\n");
		}
